...
export const getClientes = (atual, limite, loja) => {
    return function(dispatch){
        axios.get(`${api}/${versao}/api/clientes?loja=${loja}&offset=${atual}&limit=${limite}`,getHeaders())
...
export const getClientesPesquisa = (termo, atual, limite, loja) => {
    return function(dispatch){
        axios.get(`${api}/${versao}/api/clientes/search/${termo}?offset=${atual}&limit=${limite}&loja=${loja}`,getHeaders())
        .then(response => dispatch({type: GET_CLIENTES, payload: response.data}) )
        .catch(errorHandling);
    }
}
...
export const getClientePedidos = ( id, atual, limite,  loja ) =>{
    return function(dispatch){
        axios.get(`${api}/${versao}/api/clientes/admin/${id}/pedidos?offset=${atual}&limit=${limite}&loja=${loja}`,getHeaders())
